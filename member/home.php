...
<body>
	...
	if ($rows == 0)
		...
		echo "<input class='form-control' id='Fechas' type='text' name='rd_fecha' readonly='readonly' require />";
		echo "</div>";
		echo "<br /><br />";
		...
		for ($i = 0; $i < $rows; $i++) {
			...
			if ($row[6] == 1) {
				...
				echo "<td><input type='submit' name='m_request' value='Solicitar Carro' /></td>";
				echo "<td>" ?>
			...
			if (mysqli_num_rows($query->get_result()) == 1)
				...
				if (mysqli_num_rows($result) >= 3)
					...
					if ($i < $rows)
						...
						//get placa from radiobutton
						$query = $con->prepare("SELECT * FROM autos WHERE idauto = ?;");
						$query->bind_param("s", $_POST['rd_book']);
						$query->execute();
						$result = $query->get_result();
						...
						$ins = $aux . ", '" . $_SESSION['username'] . "', '" . $r['placa'] . "', '" . $_POST['rd_fecha'] . "', 'Pendiente'";
